...
import org.mav.sellingpoint.commons.EmailSender;
import org.mav.sellingpoint.commons.WorkbenchConstants;
...
import org.mav.sellingpoint.dto.ResponseEntity;
...
public class UserController {
	...
	public ResponseEntity saveUser(@Context HttpServletRequest req, User user) throws WorkbenchException, IOException {
		...
		if (user.getPassword() != null && !user.getPassword().isEmpty()) {
			...
			userDao.save(user);
			UserLog saveUserLog= createUserLog("Work Bench Admin Creating New User for WorkBench" , req.getHeader("remoteuser"), "New User Name :"+ user.getUserName(),req.getHeader("remoteemail"));
			LOG.info(saveUserLog.toString());
		}
		...
		// Send email
		try {
			createPasswordEmail(req, user, String.valueOf(notify.getId()));
	...
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public ResponseEntity updateUser(@Context HttpServletRequest req, User user) throws WorkbenchException,IOException {
		if (LOG.isDebugEnabled()) {
		...
		try {
			...
			if (null != use) {
				...
				UserLog updateUserLog= createUserLog("WorkBench Admin Updating Existing User" , req.getHeader("remoteuser"), "Updated user is :"+user.getUserName(),req.getHeader("remoteemail"));
				LOG.info(updateUserLog.toString());
				/*
	...
	@Path("/delete/{userId}")
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public ResponseEntity deleteUser(@Context HttpServletRequest req, @PathParam("userId") Integer id)
			throws WorkbenchException, IOException {
		if (LOG.isDebugEnabled()) {
			LOG.debug(">>deleteUser()");
		}
		User user = getUserDao().findUserById(id);
		...
		if (user == null) {
			...
			UserLog deleteUserLog= createUserLog("WorkBench Admin deleting Existing User" , req.getHeader("remoteuser"), "Deleted user is :"+user.getUserName(),req.getHeader("remoteemail"));
			LOG.info(deleteUserLog.toString());
			/*
			 * try { createUserLog("Update Usrer", userName,); } catch
			 * (IOException e) { e.printStackTrace(); }
			 */
	...
	@GET
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public User getUser(@PathParam("userId") Integer id) throws WorkbenchException {
		if (LOG.isDebugEnabled()) {
			LOG.debug(">>getUser()");
		}
	...
	public ResponseEntity forgotPassword(@Context HttpServletRequest req, @PathParam("email") String email) {
		...
		try {
			...
			if (user != null) {
				...
				notify = getNotificationDao().save(notify);
				// Send email
				restPasswordEmail(req, user, String.valueOf(notify.getId()));
	...
	public ResponseEntity verifyNotification(@Context HttpServletRequest req, @PathParam("id") String id)
		...
		Notification note = getNotificationDao().find(Integer.valueOf(id) , Notification.class);
		...
		try {
			if (user != null) {
				...
				if (existingUser != null) {
					...
					// delete notification
					Notification note = getNotificationDao().find(Integer.valueOf(notifyId), Notification.class);
					getNotificationDao().delete(note);
					// String body
	...
	@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public ResponseEntity addProductTuCart(@Context HttpServletRequest req) throws WorkbenchException,IOException {
		if (LOG.isDebugEnabled()) {
			LOG.debug(">>addProductTuCart()");
		}
		try {
			User use = getUserDao().getUserByName(req.getHeader("remoteuser"));
			if (null != use) {
				use.setCartCount(use.getCartCount()+1);
				userDao.saveOrUpdate(use);
			}  
			if (LOG.isDebugEnabled()) {
				LOG.debug("<<addProductTuCart()");
			}
			return new ResponseEntity(Response.Status.OK.getStatusCode(), "Item added to your Cart.", use);
		} catch (WorkbenchException _ex) {
			if (LOG.isErrorEnabled()) {
				LOG.error("--addProductTuCart() > " + _ex.getMessage(), _ex);
			}
			return new ResponseEntity(Response.Status.BAD_REQUEST.getStatusCode(), _ex.getMessage());
		}
	}
	@Path("/getLogedInUser")
	@GET
	@Produces({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML })
	public User getLogedUser(@Context HttpServletRequest req) throws WorkbenchException {
		if (LOG.isDebugEnabled()) {
			LOG.debug(">>getUserByName()");
		}
		return getUserDao().getUserByName(req.getHeader("remoteuser"));
	}
	@Autowired
	IUserDao userDao;
